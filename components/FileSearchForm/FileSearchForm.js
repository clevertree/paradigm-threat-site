...
export default function FileSearchForm({keywords}) {
    ...
    return <>
        ...
        {!keywordsList ? <h2>Enter keywords to search...</h2> : (loading ? <h2>Searching assets...</h2> : <>
                <h2>Markdown Search Results:</h2>

                {pages.length > 0 ? <ul>
                    <div className={styles.pageContainer}>
                        {pages.map(({path, lines}) => <li key={path}>
                            <a href={path}>{path}</a>
                            <p>{lines.join("\n")}</p>
                        </li>)}
                    </div>
                </ul> : <h3>No pages found</h3>}

                <h2>Asset Search Results:</h2>

                {files.length > 0 ? <div className={styles.assetContainer}>
                    {files.map((filePath) => renderAsset(filePath))}
                </div> : <h3>No assets found</h3>}
            </>
        )}
    </>;
